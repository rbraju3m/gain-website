...
<div class="grid min-h-screen lg:grid-cols-2">
        ...
        <div class="relative flex flex-1 flex-col justify-between p-12">
            ...
            <div class="max-w-md">
                ...
                <h1 class="mt-6 font-display text-4xl font-bold leading-tight sm:text-5xl">
                    Set a <span class="text-brand-green-300">new password</span>.
                </h1>
                <p class="mt-5 text-white/70">
                    Choose something strong you'll remember — at least 8 characters, with letters and numbers.
                </p>
            </div>
    ...
    <main class="flex flex-col items-center justify-center px-6 py-12 sm:px-12 lg:px-16">
        ...
        <div class="w-full max-w-md">
            <h2 class="font-display text-3xl font-bold text-brand-ink sm:text-4xl">Reset your password</h2>
            <p class="mt-2 text-brand-muted">Enter a new password to finish signing back in.</p>
            ...
            <form method="POST" action="{{ route('password.store') }}" class="mt-8 space-y-5">
                ...
                <div x-data="{ show: false }">
                    <label for="password" class="block text-xs font-semibold uppercase tracking-[0.18em] text-brand-muted">New password</label>
                    <div class="relative mt-2">
                        <span class="pointer-events-none absolute inset-y-0 left-0 grid w-10 place-items-center text-brand-muted/70">
                        ...
                        <input id="password" :type="show ? 'text' : 'password'" name="password"
                               ...
                               class="w-full rounded-xl border border-brand-ink/10 bg-white py-3 pl-10 pr-12 text-sm text-brand-ink shadow-sm transition focus:border-brand-red-300 focus:outline-none focus:ring-2 focus:ring-brand-red-200">
                        <button type="button" @click="show = !show" class="absolute inset-y-0 right-0 grid w-10 place-items-center text-brand-muted/70 hover:text-brand-red-500" :aria-label="show ? 'Hide password' : 'Show password'">
                ...
                <div>
                    <label for="password_confirmation" class="block text-xs font-semibold uppercase tracking-[0.18em] text-brand-muted">Confirm new password</label>
                    <div class="relative mt-2">
                        <span class="pointer-events-none absolute inset-y-0 left-0 grid w-10 place-items-center text-brand-muted/70">
                            <svg viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4"><path fill-rule="evenodd" d="M10 1a4 4 0 0 0-4 4v3H5a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-7a2 2 0 0 0-2-2h-1V5a4 4 0 0 0-4-4Zm-2 7V5a2 2 0 1 1 4 0v3H8Z" clip-rule="evenodd"/></svg>
                        </span>
                        <input id="password_confirmation" type="password" name="password_confirmation"
                               required autocomplete="new-password"
                               class="w-full rounded-xl border border-brand-ink/10 bg-white py-3 pl-10 pr-3 text-sm text-brand-ink shadow-sm transition focus:border-brand-red-300 focus:outline-none focus:ring-2 focus:ring-brand-red-200">
                    </div>
                </div>
