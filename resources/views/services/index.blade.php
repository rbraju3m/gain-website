...
@section('content')
    ...
    @include('partials.page-hero', [
        ...
        'accentText' => 'Services',
        'accent'     => 'red',
        'intro'      => 'Tools and resources for workplaces, employers and organisations to assess, plan and deliver effective workforce nutrition programmes.',
        'crumb'      => ['Home' => url('/'), 'Services' => null],
    ])
        ...
        <div class="mx-auto max-w-7xl px-6 lg:px-10">
            ...
            @if ($services->isEmpty())
                <div class="mt-16 rounded-3xl border border-brand-red-100 bg-brand-cream/50 px-8 py-16 text-center">
                    ...
                    <h2 class="mt-4 font-display text-2xl font-bold text-brand-ink">No services here yet</h2>
                    <p class="mt-2 text-brand-muted">{{ $active !== '' ? 'No services match this category.' : 'Check back soon — new tools land through the year.' }}</p>
                </div>
            @else
                        ...
                        <a href="{{ route('services.show', $s) }}"
                           class="card-hover reveal {{ ['', 'reveal-delay-100', 'reveal-delay-200'][$i % 3] }} group flex h-full flex-col overflow-hidden rounded-3xl bg-white shadow-card ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-soft">
                            <div class="img-zoom relative">
                                @if ($s->imageUrl())
                                    <img src="{{ $s->imageUrl() }}" alt="{{ $s->title }}" class="aspect-[16/10] w-full object-cover">
                                @else
                                ...
                                @if ($s->category)
                                    <span class="absolute left-4 top-4 inline-flex items-center gap-1.5 rounded-full bg-brand-red-500 px-3 py-1 text-xs font-semibold text-white shadow-sm">
                                        <span class="h-1.5 w-1.5 rounded-full bg-white"></span>{{ $s->category }}
                                    </span>
                                @endif
                            </div>
                            <div class="flex flex-1 flex-col p-7">
                                <h2 class="font-display text-2xl font-bold leading-tight text-brand-ink group-hover:text-brand-red-500">{{ $s->title }}</h2>
                                @if ($s->summary || $s->body)
                                    <p class="mt-3 line-clamp-3 text-brand-muted">{{ Str::limit(strip_tags($s->summary ?: $s->body), 180) }}</p>
                                @endif
