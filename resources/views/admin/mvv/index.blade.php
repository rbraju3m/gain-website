...
<div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-6 py-4">
        <div>
            ...
            <p class="text-sm text-slate-500">3-card row under the About section. {{ $cards->count() }} total · <span class="text-slate-400">drag to reorder</span>.</p>
        </div>
    ...
    @else
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100" data-sortable data-url="{{ route('admin.mvv.sort') }}">
                @foreach ($cards as $c)
                    <tr class="hover:bg-slate-50" data-id="{{ $c->id }}">
                        ...
                        <td class="px-6 py-3 text-right">
                            ...
                            <x-admin.confirm-delete :action="route('admin.mvv.destroy', $c)"
