...
<div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-6 py-4">
        <div>
            <h2 class="text-base font-semibold text-slate-900">Our Services</h2>
            <p class="text-sm text-slate-500">
                Shown in the "Services" menu and on the public services listing. {{ $services->total() }} total.
        ...
        <div class="flex flex-wrap items-center gap-3">
            <x-admin.search-box placeholder="Search services…" :value="$q" />
    ...
    @if ($services->isEmpty())
        <div class="px-6 py-12 text-center text-sm text-slate-500">
            @if (filled($q))
                No services match "<strong class="text-slate-700">{{ $q }}</strong>".
                <a href="{{ route('admin.services.index') }}" class="font-semibold text-brand-red-500">Clear search</a>.
            @else
                No services yet. <a href="{{ route('admin.services.create') }}" class="font-semibold text-brand-red-500">Create the first one</a>.
            @endif
        </div>
    @else
        <table class="w-full text-left text-sm" data-sortable-table>
            ...
            <tbody class="divide-y divide-slate-100"
                   @if (empty($q)) data-sortable data-url="{{ route('admin.services.sort') }}" @endif>
                @foreach ($services as $service)
                    <tr class="hover:bg-slate-50" data-id="{{ $service->id }}">
                        @if (empty($q))
                        ...
                        <td class="px-6 py-3">
                            @if ($service->imageUrl())
                                <img src="{{ $service->imageUrl() }}" alt="" class="h-12 w-16 rounded-lg object-cover ring-1 ring-slate-200">
                            @else
                                <div class="grid h-12 w-16 place-items-center rounded-lg bg-slate-100 text-xs text-slate-400">No image</div>
                            @endif
                        </td>
                        <td class="px-6 py-3">
                            <a href="{{ route('admin.services.edit', $service) }}" class="font-semibold text-slate-900 hover:text-brand-red-500">
                                {{ $service->title }}
                            </a>
                            <div class="text-xs text-slate-500">{{ Str::limit(strip_tags($service->summary ?: $service->body), 80) }}</div>
                        </td>
                        <td class="px-6 py-3">
                            @if ($service->category)
                        ...
                        <td class="px-6 py-3 text-right">
                            <a href="{{ route('admin.services.edit', $service) }}" class="text-sm font-semibold text-brand-red-500 hover:text-brand-red-600">Edit</a>
                            <x-admin.confirm-delete :action="route('admin.services.destroy', $service)"
                                                    title="Delete this service?"
                                                    :message="'“' . $service->title . '” will be permanently removed.'"
        ...
        @if ($services->hasPages())
            <div class="border-t border-slate-200 px-6 py-3">{{ $services->links() }}</div>
        @endif
    @endif
</div>
@endsection
